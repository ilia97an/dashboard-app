...
export async function createInvoice(prevState: State, formData: FormData) {
  ...
  try {
    ...
    `, {customerId:customerId, amountInCents:amountInCents, status:status, date:date});
...
export async function updateInvoice(id: string, prevState: State, formData: FormData) {
  ...
  try {
    await db.none(`
      ...
      WHERE id = $(id)
    `, {customerId:customerId, amountInCents:amountInCents, status:status, id:id});
  } catch (error) {
    return {
...
export async function deleteInvoice(id: string) {
  ...
  try {
    await db.none(`DELETE FROM invoices WHERE id = $(id)`, {id:id});
    revalidatePath('/dashboard/invoices');
  } catch (error) {
    return {
